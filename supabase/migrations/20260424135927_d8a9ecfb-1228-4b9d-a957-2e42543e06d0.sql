...
BEGIN
  ...
  jobs_ind AS (
    ...
    FROM live_jobs GROUP BY 1 ORDER BY 2 DESC
  ),
  jobs_role AS (
    SELECT COALESCE(NULLIF(trim(role_category), ''), 'Uncategorised') AS role, count(*) AS c
    FROM live_jobs GROUP BY 1 ORDER BY 2 DESC LIMIT 10
  ),
  top_employers AS (
    SELECT COALESCE(NULLIF(trim(company), ''), 'Unknown') AS company, count(*) AS c
    FROM live_jobs GROUP BY 1 ORDER BY 2 DESC LIMIT 15
  ),
  emp_by_ind_ranked AS (
    SELECT
      COALESCE(NULLIF(trim(industry), ''), 'Unspecified') AS industry,
      COALESCE(NULLIF(trim(company), ''), 'Unknown') AS company,
      count(*) AS c,
      row_number() OVER (
        PARTITION BY COALESCE(NULLIF(trim(industry), ''), 'Unspecified')
        ORDER BY count(*) DESC
      ) AS rn
    FROM live_jobs
    GROUP BY 1, 2
  ...
  SELECT jsonb_build_object(
    ...
    'jobsTotal', (SELECT count(*) FROM live_jobs),
