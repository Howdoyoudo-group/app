...
-- Seed superadmin role for Andrew
INSERT INTO public.user_roles (user_id, role)
SELECT id, 'superadmin'::public.app_role
FROM auth.users
WHERE lower(email) = 'andrewandtristia@gmail.com'
ON CONFLICT (user_id, role) DO NOTHING;

-- Lock admin_list_users to superadmin
CREATE OR REPLACE FUNCTION public.admin_list_users(_search text DEFAULT NULL::text, _limit integer DEFAULT 100, _offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, email text, full_name text, created_at timestamp with time zone, is_premium boolean, is_admin boolean, is_employer boolean)
 LANGUAGE sql
