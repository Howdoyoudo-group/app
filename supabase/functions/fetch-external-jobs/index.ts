...
// Shared signal filters — applied to Reed, Jooble, and JSearch results to
// prevent keyword-matched but off-topic jobs from sneaking through.
...
const INDUSTRY_SIGNALS: Record<string, { rx: RegExp; scope: "tc" | "tcd" }> = {
  ...
  books: { scope: "tc", rx: /\b(publishing|publisher|book|books|author|literary agen(?:t|cy)|book editor|fiction editor|non-fiction editor|children's editor|copy ?editor.*(book|publish)|proofreader.*(book|publish)|typesetter|bookseller|bookshop|book shop|ebook|e-book|audiobook|royalt|literary scout|book rights|foreign rights|literary fiction|penguin random house|harpercollins|hachette|pan macmillan|bloomsbury publishing|faber (?:and|&) faber|simon (?:and|&) schuster|bonnier books|canongate|profile books|waterstones|wh smith|foyles|blackwell['']?s|kindle direct publishing|kdp\b|audible|ingram content|curtis brown|united agents|society of authors|the bookseller)\b/i },
