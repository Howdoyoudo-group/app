...
import { ArrowLeft, ExternalLink, ArrowRight, Play, Headphones } from "lucide-react";
import SEO from "@/components/SEO";
import Footer from "@/components/Footer";
import PodcastGrid from "@/components/PodcastGrid";
import { getResourceTopic, type ResourceLink } from "@/data/resource-topics";
...
const ResourceCard = ({ res }: { res: ResourceLink }) => {
...
const ResourceTopic = () => {
  ...
  const topic = slug ? getResourceTopic(slug) : undefined;
  const [active, setActive] = useState<TabId>("watch");
  const navigate = useNavigate();

  // Swipe-right from anywhere to go back to /learning
  useEffect(() => {
    let startX = 0;
    let startY = 0;
    let tracking = false;
    ...
    const onEnd = (e: TouchEvent) => {
      ...
      if (dx > 80 && dy < 60) navigate("/learning");
    };
  ...
  if (!topic) return <Navigate to="/learning" replace />;
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEO
        title={`${topic.title} - Resources`}
        description={topic.description}
        path={`/resources/${topic.slug}`}
      />
