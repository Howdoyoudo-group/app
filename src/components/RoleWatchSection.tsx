...
const ncsVideoId = (url: string | null) => {
  if (!url) return null;
...
const RoleWatchSection = ({ roleName, roleSlug }: RoleWatchSectionProps) => {
  ...
  const [ncsVideoUrl, setNcsVideoUrl] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    supabase
      .from("role_metadata")
      .select("ncs_video_url")
      .eq("slug", roleSlug)
      .maybeSingle()
      .then(({ data }) => {
        if (cancelled) return;
        setNcsVideoUrl((data as any)?.ncs_video_url ?? null);
      });
    return () => { cancelled = true; };
  }, [roleSlug]);

  const ncsId = ncsVideoId(ncsVideoUrl);
      ...
      {ncsId && (
        ...
        >
          <div className="relative">
            <img
              src={`https://i.ytimg.com/vi/${ncsId}/hqdefault.jpg`}
              alt={`Meet a ${roleName}`}
              loading="lazy"
          ...
          <div className="p-4">
            ...
            <h3 className="font-display font-700 text-base text-foreground leading-tight mt-1 group-hover:text-primary transition-colors">
              Meet a real {roleName.toLowerCase()}
            </h3>
