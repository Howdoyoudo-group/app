...
const INSPIRE: DropdownItem[] = [
  { label: "HDYD Originals", to: "/the-show", description: "Episodes, pitches, street interviews & photos" },
  { label: "Stuff We Rate", to: "/articles", description: "Read & learn" },
...
const SiteHeader = ({ overlay = false, showLogo }: SiteHeaderProps) => {
  ...
  useEffect(() => {
    ...
    if (!user) {
      ...
      setCuriosityActiveCategories(null);
      return;
    }
    (async () => {
      ...
      const { data } = await supabase
        .from("profiles")
        .select("photo_url, full_name, curiosity_score, curiosity_breadth, curiosity_active_categories")
        .eq("id", user.id)
        .maybeSingle();
      if (!active) return;
