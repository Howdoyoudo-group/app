...
import { Loader2, Plus, X, Heart, ImagePlus } from "lucide-react";
...
interface Props {
  userId: string;
  photos: LovePhoto[];
  onChange: (next: LovePhoto[]) => void;
}

const LovesGallery = ({ userId, photos, onChange }: Props) => {
  const fileRef = useRef<HTMLInputElement>(null);
  const [uploading, setUploading] = useState(false);
  const [pendingCategory, setPendingCategory] = useState<string>("Bingeworthy TV");
  ...
  const updatePhoto = (idx: number, patch: Partial<LovePhoto>) => {
  ...
  return (
    <div>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {photos.map((photo, idx) => (
          ...
          >
            ...
            <div className="absolute inset-x-0 top-0 p-2 flex items-start justify-between gap-2">
              ...
              >
                {[photo.category || "Love", ...CATEGORY_SUGGESTIONS.filter(c => c !== (photo.category || "Love"))].map((c) => (
                  <option key={c} value={c}>{c}</option>
                ))}
              </select>
              {/* Static category pill for PDF/print export (replaces the <select>) */}
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-foreground/95 via-foreground/70 to-transparent p-2 pt-8">
              ...
              />
              {/* Static caption text for PDF/print */}
              {photo.caption && (
        ...
        {/* Upload tile - hidden in PDF / print */}
        <button
          type="button"
      ...
      {/* Category picker for next upload - hidden in PDF / print */}
