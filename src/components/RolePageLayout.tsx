...
const RolePageLayout = ({ name, description, tabs, category, slug }: RolePageLayoutProps) => {
  ...
  const renderTabBox = (tab: RoleTab, compact: boolean) => {
        ...
        <div className="hidden md:block fixed top-0 inset-x-0 z-40 bg-background border-b-2 border-foreground/10 shadow-sm">
          <div className="container mx-auto px-6 md:px-12">
            <div className="grid grid-cols-9 gap-3 py-3">
              {enhancedTabs.map((tab) => renderTabBox(tab, true))}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="container mx-auto px-6 md:px-12 py-12 md:py-20">
        ...
        >
          ...
          <div className="grid grid-cols-3 md:grid-cols-9 gap-3 mb-12 border-b border-border pb-6">
            {enhancedTabs.map((tab) => renderTabBox(tab, false))}
          </div>
          {/* Sentinel the sticky-bar observer watches - see note above. */}
          <div ref={tabBarSentinelRef} />
        </motion.div>

        {/* Padded down when a sticky bar is showing so content doesn't sit
            under it (the bar is `fixed`, so it overlays rather than pushing
            content down) - values match each bar's measured height with a
            little headroom. */}
        <motion.div
          ref={contentRef}
          key={activeTab}
